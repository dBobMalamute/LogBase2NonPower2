                                                               ...
                                                               CasingBackend* outCasingBackend, PlugNumber outPlugNumber)
{
    ...
    inCasomgBackend->casingState().setWire(PlugType::IN, inPlugNumber, *wireBackend);
    outCasingBackend->casingState().setWire(PlugType::OUT, outPlugNumber, *wireBackend);
...
QJsonArray CorkboardBackend::removeCasingBackend(CasingBackend *casingBackend)
{
    ...
    for(auto plugType: {PlugType::IN,PlugType::OUT})
    {
...
{
    QColor color = c.getCasing(PlugType::OUT)->casing().getOutPortColor(c.getPlugIndex(PlugType::OUT));
    c.getCasing(PlugType::IN)->casing().setInPortColor(c.getPlugIndex(PlugType::IN), color);
...
{
    c.getCasing(PlugType::IN)->casing().setInPortToDefaultColor(c.getPlugIndex(PlugType::IN));
}
