...
bool WireCasingInteraction::tryConnect() const
{
    ...
    PlugNumber inPlugNumber = m_wireBackend->getPlugIndex(PlugType::IN);
    PlugNumber outPlugNumber = m_wireBackend->getPlugIndex(PlugType::OUT);
    auto inCasingBackend = m_wireBackend->getCasing(PlugType::IN);
    auto outCasingBackend = m_wireBackend->getCasing(PlugType::OUT);
    
    inCasingBackend->casingState().setWire(PlugType::IN, inPlugNumber, *m_wireBackend);
    m_wireBackend->getWire()->setColor(outCasingBackend->casing().outPlugLabelColors().at(outPlugNumber));
    
    outCasingBackend->casingState().setWire(PlugType::OUT, outPlugNumber, *m_wireBackend);
