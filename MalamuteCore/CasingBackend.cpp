...
{
    for (PlugType plugType: {PlugType::IN, PlugType::OUT})
    {
...
void CasingBackend::onNewData(PlugNumber index)
{
    ...
    auto wires = m_casingState.wires(PlugType::OUT, index);
    for (auto const & c : wires)
    {
        c.second->sendData(ideaData);
...
void CasingBackend::onNewDataAllPlugs()
{
    ...
    auto wiresVector = m_casingState.wires(PlugType::OUT);
